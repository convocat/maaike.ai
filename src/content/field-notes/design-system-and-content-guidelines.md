...
- **Project hubs are always field notes.** If it has `hub: true`, it lives in `field-notes/`. Seeds never get `hub` or `project` tags.
...
Project hubs are field notes with `hub: true` that track a multi-item effort. They follow this structure:
...
Motivation. What triggered this? What's the gap?
...
- Standard deliverables for every project: a **write-up report** (article or field note summarizing findings) and a **slide deck** (for video or presentation)
