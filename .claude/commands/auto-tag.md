...
2. **All post titles and slugs**: For each collection (articles, field-notes, sparks, weblinks, videos, library, principles), glob `src/content/<collection>/*.md` and extract `title` from frontmatter. Build a map of title-to-slug for wiki-link candidates.
