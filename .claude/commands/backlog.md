...
**Inbox items**: check `src/content/_inbox/` for `.md` or `.txt` files. The inbox is a persistent running note with date-stamped entries. Entries prefixed with `✅` are already processed — skip them. Surface only unprocessed entries prominently — they are the highest-priority quick win. Never delete the inbox file.
